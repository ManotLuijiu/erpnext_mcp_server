...
# ─────────────────────────────────────────────────────────────────────
# Erpnext Mcp Server custom image for Frappe Docker
#
...
#
#   export CUSTOM_IMAGE=bunchee/erpnext_mcp_server
...
ARG APP_REPO=https://github.com/ManotLuijiu/erpnext_mcp_server
...
RUN apk add --no-cache git \
 ...
 && cd /tmp/app \
 && (git log -1 --format='erpnext_mcp_server @ %H | %s | %an <%ae>' > /tmp/app_version.txt) \
...
LABEL org.opencontainers.image.title="Erpnext Mcp Server" \
      org.opencontainers.image.description="MCP server with OKF knowledge layer" \
      org.opencontainers.image.source="https://github.com/ManotLuijiu/erpnext_mcp_server" \
...
COPY --chown=frappe:frappe --from=fetcher /tmp/app /home/frappe/frappe-bench/apps/erpnext_mcp_server
COPY --chown=frappe:frappe --from=fetcher /tmp/app_version.txt /home/frappe/frappe-bench/apps/erpnext_mcp_server_version.txt
...
RUN bench setup requirements \
 ...
 && (python -c "import erpnext_mcp_server; print('Erpnext Mcp Server:', getattr(erpnext_mcp_server, '__version__', 'unknown'))" || echo "  (skip per-app import — module name may differ)") \
 && cat /home/frappe/frappe-bench/apps/erpnext_mcp_server_version.txt \
 && echo "✅ Erpnext Mcp Server image built from ${APP_REF}"
